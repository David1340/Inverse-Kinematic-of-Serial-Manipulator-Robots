...
L = 360 #largura do quadrado de pesquisa
...
for i in range(M):
    x = round(random.random()*360)
    y = round(random.random()*360)   
    q[0,i] = x
    q[1,i] = y
...
#calcular a melhor particula
    ...
    py = [q[1,0],q[1,1],q[1,2],q[1,3],q[1,4],q[1,5],q[1,6],q[1,7],q[1,8],q[1,9]]
    plt.scatter(px,py)
    plt.xlabel('Teta1(graus)')
    plt.ylabel('Teta2(graus)')
    plt.legend(['Objetivo','Limite do espaço de busca','Particulas'])
    plt.title('GA')
    fig.canvas.draw()
    plt.pause(0.1)
    plt.clf()
