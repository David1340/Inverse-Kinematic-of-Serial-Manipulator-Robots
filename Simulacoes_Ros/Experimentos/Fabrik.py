...
def restringe_junta(p1,p2,p3,i):
    ...
    if(th < pi - qlim[i]):
        v1 = rotationar_vetor(v1,v3,th-(pi - qlim[i] + 0.001))
        p1 = vetor(p2) + d*v1
    else:
        p1 = vetor(p1)
    return p1[:,0]
...
def restringe_junta2(v1,v2,i): 
    ...
    if(th > qlim[i]):
        v1 = rotationar_vetor(v2,v3,qlim[i]-0.001)
    return v1[:,0]/norm(v1)
...
print('convergiu ',convergencia/10,' vezes')
print('excedeu',excedeu, 'vezes')
print('media de iteracoes:', ktotal/convergencia)
