...
#dada uma posição (x,y,z) e uma orientação (rpyulos de euler)
...
from funcoes import orientacao
...
#Calcula os rpyulos de RPY a partir de uma matriz de Rotacao
def orientacao(A):
...
for teste in range(1000):
    ...
    #Inicialização dos rpyulos de juntas
    q = np.zeros([7,1])
    for a in range(np.size(q)):
        q[a] = random.uniform(-qlim[a],qlim[a])
    ...
    while not rospy.is_shutdown():
        for cont in range(1000):
            ...
            #atualiaza os rpyulos do manipulador no Rviz
            #hello_str.header.stamp = rospy.Time.now()
            ...
            #a parte rpyular peguei do artigo A closed-loop inverse kinematic scheme
